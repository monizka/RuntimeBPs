...
struct FNodeVarArgs
{
	...
	NODE_VAR_ARG_FUNCTIONS(UDamageType*, DamageType, nullptr) // 26

	NODE_VAR_ARG_FUNCTIONS(FHitResult, HitResult, FHitResult()) // 27
	...
	FORCEINLINE bool operator==(const FNodeVarArgs& Other) const
	{
		switch(GetSubtypeIndex())
		{
			...
			case 26: return GetDamageTypeArg() == Other.GetDamageTypeArg();
			case 27: return false; // HitResult has no equality comparison, we could make one ourselves but in most cases it's false anyway.
